...
**Layout split: title bar + two columns + bottom strip.** `_render_panel_images()` lays the panel out as: a red title bar (`Air Quality - <city>` / `AQI Rising!` / `Air Quality [CACHED]`), a hairline divider, a left column of compact stat rows (PM2.5 with trend marker, PM10, Temp, Humid), a right column with the hero AQI, and a bottom strip that shares an AQI gauge bar with the timestamp. The gauge is an outlined rect with a red fill proportional to `min(AQI, 300) / 300` — saturated = "at least very unhealthy." On `stale=True` the gauge intentionally renders outline-only (no red fill) so a `[CACHED]` panel reads as obviously not-fresh at a glance. The borders on both layers shrank from width=3 to width=1 in this redesign; `tests/test_render.py::test_borders_present_on_both_layers` still asserts an inked pixel at `(3,3)` on black and `(5,5)` on red, so keep at least a 1-pixel rect at those coords if you rework the frame again.
...
- `docs/generate_preview.py` — regenerates `docs/preview.png` after layout changes; thin wrapper that calls `airQuality.render_preview_png()` with dummy data so there's no parallel copy of the draw code to keep aligned
